...
export default function StudentPaymentPage() {
  ...
  const loadPaymentInfo = async () => {
    try {
      ...
      // 1. Try the dedicated amount endpoint (admin-configured fee)
      try {
        const amountRes = await paymentAPI.getAmount();
        const amount = amountRes.data?.amount || amountRes.data?.data?.amount || 0;
        if (amount > 0) resolvedAmount = amount;
      } catch {
        // Endpoint may be restricted to admin role; will fall back to status below
      }

      // 2. Load payment status - also use as amount fallback if above failed
      try {
        const statusRes = await paymentAPI.getStatus();
        const statusData = statusRes.data.data || statusRes.data;
        const mappedPaymentInfo = {
          ...statusData,
          status: statusData.paymentStatus || statusData.status,
          reference: statusData.paymentCode || statusData.reference,
          datePaid: statusData.datePaid || statusData.paidAt,
        };
        setPaymentInfo(mappedPaymentInfo);
        // Use amount embedded in status response if we still don't have one
  ...
  const handlePayment = async () => {
    // If there's already a pending payment, confirm before creating a new one
    ...
    try {
      ...
      // After payment initialization, show payment code dialog instead of redirecting immediately
      ...
      if (error.response?.status === 400) {
        ...
        if (backendMessage.includes('already completed payment') || backendMessage.includes('already paid')) {
          ...
          // Reload payment info to show updated status
          loadPaymentInfo();
          return;
        }
  ...
  const getStatusBadge = (status: string) => {
    // Normalize status - backend might return 'completed', 'paid', 'success'
                ...
                <div className="space-y-2">
                  ...
                  <p className="text-sm">
                    ...
                    {paymentInfo?.reference && (
                      <span> Reference: <code className="bg-orange-100 px-1 rounded">{paymentInfo.reference}</code></span>
                    )}
                  </p>
                  <div className="flex gap-2 mt-3">
                    ...
                    <Button 
                      size="sm"
                      onClick={() => router.push('/student/payment/verify')}
          ...
          {!reservation && (
            <Alert className="bg-blue-50 border-blue-200">
              ...
              <AlertDescription className="text-blue-800">
                You need to reserve a room before making payment. 
                <Button 
                  variant="link" 
                  className="p-0 h-auto ml-1 text-blue-800"
           ...
           reservation && (
            <Card>
              <CardHeader>
                ...
                <CardDescription>
                  Pay securely using your debit/credit card or bank transfer
                </CardDescription>
              </CardHeader>
              <CardContent className="space-y-4">
                <div className="bg-muted rounded-lg p-4 space-y-2">
                  <div className="flex items-center justify-between">
                    ...
                    <span className="text-2xl font-bold">₦{paymentAmount.toLocaleString()}</span>
                  </div>
                </div>
