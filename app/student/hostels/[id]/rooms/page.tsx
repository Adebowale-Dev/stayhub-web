...
interface Room {
  ...
  roomNumber: string;
  floor: number;
  capacity: number;          // actual bunk count (real max bookable slots)
  currentOccupants: number;  // reserved + occupied — updates on reservation
  availableSpaces: number;   // always accurate available bunk count
  reservedSpaces: number;    // bunks pending check-in
  isAvailable: boolean;      // true only if at least one bunk is free
  gender: string;
  amenities?: string[];
}
...
export default function HostelRoomsPage() {
  ...
  // Extract loadRooms as a reusable function
  const loadRooms = async () => {
    ...
    try {
      ...
      // Backend now sends accurate real-time bunk data — trust it directly
      const mappedRooms: Room[] = roomsData.map((room: any) => ({
        _id:              room._id,
        roomNumber:       room.roomNumber,
        floor:            room.floor,
        capacity:         room.capacity,          // actual bunk count
        currentOccupants: room.currentOccupants,  // reserved + occupied
        availableSpaces:  room.availableSpaces,   // always accurate
        reservedSpaces:   room.reservedSpaces ?? 0,
        isAvailable:      room.isAvailable,       // true if at least one bunk free
  ...
  const filteredRooms = rooms.filter((room) => {
    ...
    // Availability filter - use isAvailable which comes from backend's availableSpaces
  ...
  const validateAndAddFriend = async () => {
    ...
    // Check room capacity against real-time availableSpaces from backend.
    // Only count the friends' slots — the current user's slot is already deducted
    // from availableSpaces (if they reserved earlier) or validated by the backend on submit.
    const totalFriendsAfterAdding = invitedFriends.length + 1; // friends already added + this new friend
    ...
    try {
      // TODO: Call API to validate friend by matric number
      // For now, we'll use a placeholder that shows the matric number
      // Backend endpoint needed: GET /api/student/validate-friend/:matricNo
      // Should return: { _id, firstName, lastName, matricNo, gender, hasReservation }
      
      // Temporary: Create a placeholder (remove this when backend is ready)
      const tempFriend = {
        _id: `temp-${Date.now()}`,
        matricNo: friendInput.toUpperCase(),
        name: `Student (${friendInput})`, // Will be replaced by actual name from backend
      };
  ...
  const confirmReservation = async () => {
    if (!selectedRoom || !hostelId) return;
    ...
    try {
      ...
      // Add friends array if group reservation
      ...
      const axiosError = error as { response?: { status?: number; data?: { message?: string; error?: string; details?: string } }; message?: string };
      ...
      if (status === 403 || errorMessage.toLowerCase().includes('payment required')) {
        // Close the reservation dialog before redirecting
        resetReservationDialog();
    ...
    <ProtectedRoute allowedRoles={['student']}>
      <DashboardLayout>
          ...
          <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4">
            {filteredRooms.length === 0 ? (
              ...
              filteredRooms.map((room) => {
                const availableSpaces = room.availableSpaces; // accurate from backend
                const hasSpace = room.isAvailable; // true only if at least one bunk is free

                return (
                  <Card 
                    key={room._id} 
                    className={`hover:shadow-lg transition-shadow ${!hasSpace ? 'opacity-60' : ''}`}
                  >
                    <CardHeader className="pb-3">
        ...
        <Dialog open={reserveDialogOpen} onOpenChange={(open) => {
          if (!open) resetReservationDialog();
          else setReserveDialogOpen(open);
        }}>
          <DialogContent className="max-w-2xl max-h-[90vh] overflow-y-auto">
              ...
              <div className="space-y-4 py-4">
                ...
                <div className="space-y-4">
                  <div className="flex items-center space-x-2">
                    <Checkbox 
                      id="reserve-with-friends"
                      checked={reserveWithFriends}
                  ...
                  {reserveWithFriends && (
                    <div className="space-y-3 border rounded-lg p-4 bg-muted/30">
                      ...
                      <div className="flex gap-2">
                        <Input
                          placeholder="Enter matric number (e.g., CSC/2020/001)"
                          value={friendInput}
                        ...
                        <Button
                          type="button"
                          onClick={validateAndAddFriend}
                          disabled={validatingFriend || !friendInput.trim()}
                          size="sm"
                        >
                            ...
                            {invitedFriends.map((friend) => (
                              ...
                              >
                                ...
                                <Button
                                  variant="ghost"
                                  size="sm"
                                  onClick={() => removeFriend(friend.matricNo)}
            ...
            <DialogFooter>
              <Button
                variant="outline"
                onClick={() => resetReservationDialog()}
                disabled={reserving}
              >
                Cancel
              </Button>
